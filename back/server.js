...
app.use(cors())
console.log(`${loadFile('2025-08-29')} lines loaded`)
...
app.get('/load/:folder', (req, res) => {
    lines = []
    fs.createReadStream(`${baseFolder}/${req.params.folder}/index.txt`).on('error', (err) => {
...
function loadFile(date) {
    lines = []
    fs.createReadStream(`${baseFolder}/${date}/index.txt`).on('error', (err) => {
        res.status(404).json(err)
    })
        .pipe(es.split())
        .pipe(es.map(function (line, next) {
